...
export function Modal({ open, title, description, children, footer, onClose, className = '' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
      <section
        className={[
          'flex max-h-[86vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-stone-200 bg-white text-slate-800 shadow-2xl',
          className,
        ].join(' ')}
      >
        ...
        {footer ? <footer className="border-t border-stone-200 bg-stone-50 px-5 py-4">{footer}</footer> : null}
      </section>
    </div>
  );
}
